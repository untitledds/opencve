...
@pytest.mark.django_db
def test_filter_by_vendor(create_cve, auth_client):
    """
    Тест для проверки фильтрации CVE по вендору.
    """
    client = auth_client()

    # Создаем несколько CVE
    create_cve("CVE-2022-20698")  # Вендоры: clamav, canonical, debian
    create_cve("CVE-2022-22965")  # Вендоры: vmware, oracle, redhat

    # Фильтруем по вендору 'vmware'
    response = client.get(reverse("extended-cve-list") + "?vendor=vmware")

    # Проверяем статус ответа
    assert response.status_code == status.HTTP_200_OK

    # Проверяем, что возвращен только CVE-2022-22965
    cve_ids = [cve["cve_id"] for cve in response.json()["results"]]
    assert cve_ids == ["CVE-2022-22965"]


@pytest.mark.django_db
def test_filter_by_product(create_cve, auth_client):
    """
    Тест для проверки фильтрации CVE по продукту.
    """
    client = auth_client()

    # Создаем несколько CVE
    create_cve("CVE-2022-20698")  # Продукты: clamav, ubuntu_linux, debian_linux
    create_cve("CVE-2022-22965")  # Продукты: spring_framework, weblogic_server

    # Фильтруем по продукту 'spring_framework'
    response = client.get(reverse("extended-cve-list") + "?product=spring_framework")
...
@pytest.mark.django_db
def test_filter_by_date(create_cve, auth_client):
    """
    Тест для проверки фильтрации CVE по дате обновления.
    """
    client = auth_client()

    # Создаем несколько CVE
    create_cve("CVE-2022-20698")  # updated: 2024-11-06T16:32:32.016000+00:00
    create_cve("CVE-2022-22965")  # updated: 2024-07-31T20:10:19.936000+00:00

    # Фильтруем по дате обновления после 2024-08-01
    response = client.get(reverse("extended-cve-list") + "?start_date=2024-08-01")

    # Проверяем статус ответа
    assert response.status_code == status.HTTP_200_OK

    # Проверяем, что возвращен только CVE-2022-20698
    cve_ids = [cve["cve_id"] for cve in response.json()["results"]]
    assert cve_ids == ["CVE-2022-20698"]


@pytest.mark.django_db
def test_filter_by_cvss(create_cve, auth_client):
    """
    Тест для проверки фильтрации CVE по CVSS.
    """
    client = auth_client()

    # Создаем несколько CVE
    create_cve("CVE-2022-20698")  # CVSS: 7.5 (High)
    create_cve("CVE-2022-22965")  # CVSS: 9.8 (Critical)

    # Фильтруем по CVSS >= 9.0 (Critical)
    response = client.get(reverse("extended-cve-list") + "?cvss=critical")
